...
def main():
    ...
    train_dataloader = my_dataloader.my_dataloader_DDP(root='/data/mxj/IVF_HUST/first_travel_VideoImage',
                                                       filename='train.json', args=args, train=True)
    val_dataloader = my_dataloader.my_dataloader_DDP(root='/data/mxj/IVF_HUST/first_travel_VideoImage',
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (blobs, targets) in train_dataloader:
        ...
        if not args.val_test:
            ...
            if metrics.accuracy_score(test_target_list, test_predict_list) > 0.715:
                savemodel.save_model(net, this_train_tag, 'val_acc_{}_{}.pth'.format(metrics.accuracy_score(test_target_list, test_predict_list),my_save_flag),
                                     epoch, lr, parallel_tag, val_acc=best_val_acc)
                my_save_flag += 1
