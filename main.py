...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if not args.val_test:
            ...
            test_predict_list, test_target_list, test_predict_prob = [], [], []
            test_loss_total = 0
            for batch_idx, (blobs, targets) in enumerate(test_dataloader):
                ...
                with torch.no_grad():
                    if args.net == 'Ada':
                        pred_list = net(blobs)
                        if not args.adamode == 'bi':
                            test_loss_strategy = loss(pred_list, cls_targets, args)
                        else:
                            loss1 = loss(pred_list[0:3], cls_targets, args)
                            loss2 = loss(pred_list[3:], cls_targets, args)
                            test_loss_strategy = loss1 + loss2
                        if args.cell_pred:
                            final_pred = pred_list[1]
                            if args.adamode == 'bi':
                                final_pred = pred_list[1] + pred_list[4]
                        ...
                        cls_preds = net(blobs)
                        test_loss_strategy = loss(cls_preds, cls_targets)
                        # TODO: mixup in loss function
                ...
                test_loss_total = test_loss_total + test_loss_strategy

                utils.progress_bar(batch_idx, len(val_dataloader), 'Loss: %.5f | Acc: %.3f'
                                   % (test_loss_total / (batch_idx + 1),
                                      100. * metrics.accuracy_score(test_target_list, test_predict_list)))

            tensorboard_writer.write_scalars(test_loss_total / len(test_dataloader),
                                             lr,
                                             [test_target_list, test_predict_list, test_predict_prob],
                                             n_iter=epoch,
                                             tag='test', auc=True)
            if metrics.accuracy_score(test_target_list, test_predict_list) > best_t_acc:
                best_t_acc = metrics.accuracy_score(test_target_list, test_predict_list)
                # print('Saving best model, acc:{}'.format(best_val_acc))
                savemodel.save_model(net, this_train_tag, 'best_t_acc.pth',
                                     epoch, lr, parallel_tag, val_acc=best_t_acc)

            # # =========================================== mix ===========================================
            # mix_loss_total = test_loss_total + val_loss_total
            # mix_len_total = len(val_dataloader) + len(test_dataloader)
            # mix_target_list = test_target_list + val_target_list
            # mix_predict_list = test_predict_list + val_predict_list
            # mix_predict_prob = test_predict_prob + val_predict_prob
            # tensorboard_writer.write_scalars(mix_loss_total / mix_len_total,
            #                                  lr,
            #                                  [mix_target_list, mix_predict_list, mix_predict_prob],
            #                                  n_iter=epoch,
            #                                  tag='test-and-val-mix', auc=True)
