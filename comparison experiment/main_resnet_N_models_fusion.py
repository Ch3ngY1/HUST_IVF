...
class MyResNet(nn.Module):
    def __init__(self, frames=1):
        ...
        self.feature = nn.Sequential(*list(self.basemodel.children())[:-1])
...
class MyOverAllResNet(nn.Module):
    def __init__(self, num, frames):
        super(MyOverAllResNet, self).__init__()
        self.num = num
        self.frames = frames
        self.basemodel = []

        self.base0 = MyResNet(frames=frames)
        self.base1 = MyResNet(frames=frames)
        self.base2 = MyResNet(frames=frames)
        self.base3 = MyResNet(frames=frames)
        self.base4 = MyResNet(frames=frames)

        self.fc = nn.Linear(512*4, 2)
        self.pool = nn.AvgPool2d(kernel_size=[1,num], stride=1)
    def forward(self, input):
        # input = bs * num * seg * 224 * 224
        x0 = self.base0(input[:,0,...]).squeeze()
        x1 = self.base0(input[:,1,...]).squeeze()
        x2 = self.base0(input[:,2,...]).squeeze()
        x3 = self.base0(input[:,3,...]).squeeze()
        x4 = self.base0(input[:,4,...]).squeeze()

        out = torch.stack([x0, x1, x2, x3, x4], dim=2)
...
if __name__ == '__main__':
    ...
    net = MyOverAllResNet(num=5, frames=7)
    # net.froze_bn()
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (blobs, targets) in enumerate(train_dataloader):
            ...
            utils.progress_bar(batch_idx, len(train_dataloader), 'Lr: %.5f | Loss: %.3f | Acc: %.3f'
                               % (lr, train_loss_total / (batch_idx + 1), 100. * metrics.accuracy_score(train_target_list, train_predict_list)))
            # break
        '''
