...
def my_transformer(train, modality, new_length, size=224):
    ...
    transformer = torchvision.transforms.Compose([
        ...
        augmentation,
        pre_process,
        ToTorchFormatTensor(size, div=True,
...
class ToTorchFormatTensor(object):
    ...
    def __init__(self, size, div=True, modality='RGB', new_length=1):
        self.div = div
    ...
    def __call__(self, pic):
        if isinstance(pic, np.ndarray):
            ...
            if self.modality in ['RGB', 'FlowPlusGray']:
                img = img.view(3 * self.new_length, -1, self.size, self.size)
            elif self.modality == 'CSP':
                img = img.view(-1, self.new_length, self.size, self.size)
                img = img.permute(1, 0, 2, 3)
            elif self.modality in ['SPECTRUM', 'CANCELLED', 'GRAY']:
                img = img.view(self.new_length, -1, self.size, self.size)
            elif self.modality == 'AdapPool':
                img = img.view(-1, self.new_length, self.size, self.size)
            elif self.modality == 'VideoGray':
                img = img.view(-1, self.size, self.size)
                ...
                img = img.view(2 * self.new_length, -1, self.size, self.size)
        else:
            # handle PIL Image
