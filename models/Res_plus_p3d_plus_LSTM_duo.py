...
class Ada(nn.Module):
    def __init__(self, init_function, num_classes=2, num_frame=96, position_mode='add', c_trans=False):
        ...
        self.lstm2 = myLSTM(num_frame=num_frame, position_mode=position_mode, c_trans=self.c_trans)
    ...
    def forward(self, input):
        ...
        N, f, _ = lstmin.shape
        lstmin2 = torch.stack([lstmin[:,f-i-1,:] for i in range(f)],dim=1)
        hidden2, cell2, utility2 = self.lstm2(lstmin2)

        return hidden, cell, utility, hidden2, cell2, utility2
