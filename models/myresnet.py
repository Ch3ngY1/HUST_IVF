...
class myResNet(nn.Module):
    def __init__(self, block, layers, num_classes, head):
        ...
        m = OrderedDict()
        m['conv1'] = nn.Conv2d(128, 128, kernel_size=7, stride=2, padding=3, bias=False)
        m['bn1'] = nn.BatchNorm2d(128)
        m['relu1'] = nn.ReLU(inplace=True)
        ...
        self.channels_per_layer = (np.array([128, 128, 256, 512]) * block.expansion).tolist()
...
def myresnet18(num_classes=1000, init_function=None, head=True):
    model = myResNet(BasicBlock, [2, 2, 2, 2], num_classes, head)
    model.apply(init_function)
    return model

def myresnet34(num_classes=1000, init_function=None, head=True):
    model = myResNet(BasicBlock, [3, 4, 6, 3], num_classes, head)
    model.apply(init_function)
    return model

def myresnet50(num_classes=1000, init_function=None, head=True):
    model = myResNet(Bottleneck, [3, 4, 6, 3], num_classes, head)
    model.apply(init_function)
    return model

def myresnet101(num_classes=1000, init_function=None, head=True):
    model = myResNet(Bottleneck, [3, 4, 23, 3], num_classes, head)
    model.apply(init_function)
    return model

def myresnet152(num_classes=1000, init_function=None, head=True):
    model = myResNet(Bottleneck, [3, 8, 36, 3], num_classes, head)
    model.apply(init_function)
    return model