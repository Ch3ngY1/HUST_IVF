...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        if modality == 'rgb':
            in_channels = 3 * new_length
        elif modality == 'flow':
            in_channels = 2 * new_length
        elif modality == 'gray':
            in_channels = 1 * new_length
        else:
            raise ValueError(
    ...
    def load_model(self, model_path, input_conv=True):
        pre_model = torch.load(model_path)
        new_state = self.state_dict()
        for k, v in pre_model.items():
            if (k in new_state) and ('conv3d_0c_1x1' not in k):
                if 'conv3d_1a_7x7.conv3d' in k:
                    new_value = torch.mean(v, 1, keepdim=True).expand(-1, self.in_channels, -1, -1, -1)
                    new_state[k] = new_value
                else:
                    new_state[k] = v
            else:
                print('{} not load to model'.format(k))
                pass

        # pre_model_dict = {k: v for k, v in pre_model.items() if ('conv3d_0c_1x1' not in k)}
...
def i3d(num_classes, init_function, modality, new_length, **kwargs):
    """construct a P3D199 model based on a ResNet-152-3D model.
    """
    model = I3D(num_classes, modality, new_length, **kwargs)
